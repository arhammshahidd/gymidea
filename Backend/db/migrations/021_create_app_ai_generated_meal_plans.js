exports.up = async function(knex) {
  await knex.schema.createTable('app_ai_generated_meal_plans', (table) => {
    table.increments('id').primary();
    table.integer('request_id').notNullable().references('id').inTable('app_ai_meal_plan_requests').onDelete('CASCADE');
  ...
  await knex.schema.createTable('app_ai_generated_meal_plan_items', (table) => {
    table.increments('id').primary();
    table.integer('plan_id').notNullable().references('id').inTable('app_ai_generated_meal_plans').onDelete('CASCADE');
...
exports.down = async function(knex) {
  await knex.schema.dropTableIfExists('app_ai_generated_meal_plan_items');
  await knex.schema.dropTableIfExists('app_ai_generated_meal_plans');
};
