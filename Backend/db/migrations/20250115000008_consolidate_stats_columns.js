/**
 * Consolidate all stats TEXT columns into a single JSONB column
 * This reduces storage and improves performance
 * @param { import("knex").Knex } knex
 * @returns { Promise<void> }
 */
exports.up = async function(knex) {
  // First, add a new JSONB column for consolidated stats
  ...
  // Migrate existing stats data to the consolidated column
  ...
  // Now drop the individual stats columns
  await knex.schema.alterTable('daily_training_plans', (table) => {
...
exports.down = async function(knex) {
  ...
  await knex.schema.alterTable('daily_training_plans', (table) => {
    table.timestamp('stats_date_updated').nullable();
  ...
  // Migrate data back from stats_data
  const statsRecords = await knex('daily_training_plans')
    .where('is_stats_record', true)
