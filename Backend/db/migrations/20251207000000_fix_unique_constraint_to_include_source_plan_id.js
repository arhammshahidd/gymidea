 ...
 * This allows users to have multiple plans of the same type on the same day_number
 * if they come from different assignments (different source_plan_id)
 * 
 * Example: User can have 2 "web_assigned" plans on Day 1:
...
exports.up = async function(knex) {
  ...
  // This allows multiple plans with same (user_id, day_number, plan_type) 
  ...
  // - This allows: 2 "web_assigned" plans on same day_number from different assignments
  await knex.raw(`
  ...
  console.log('   Users can now have multiple plans of the same type on the same day_number');
...
exports.down = async function(knex) {
  ...
  await knex.raw(`
    CREATE UNIQUE INDEX daily_training_plans_user_plan_unique 
    ON daily_training_plans (user_id, day_number, plan_type) 
    WHERE is_stats_record = false AND day_number IS NOT NULL;
  `);
